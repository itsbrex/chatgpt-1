...
const Form = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const localRes: any = storedResponse ? JSON.parse(storedResponse) : null;
    ...
    const response = await fetch("/api/response", {
      ...
      body: JSON.stringify({
        message: localRes ? [...localRes, message] : [message],
        currentModel,
      }),
    });
    ...
    if (localRes) {
      localStorage.setItem(
        "response",
        JSON.stringify([...localRes, message, currentResponse.join("")])
      );
    } else {
      localStorage.setItem(
        "response",
        JSON.stringify([message, currentResponse.join("")])
      );
    }
  ...
  return (
    <div className="flex justify-center">
      ...
      <form
        ...
        className="fixed bottom-0 w-full md:max-w-3xl rounded-md shadow-[0_0_10px_rgba(0,0,0,0.10)] mb-4"
      >
        <textarea
          name="Message"
